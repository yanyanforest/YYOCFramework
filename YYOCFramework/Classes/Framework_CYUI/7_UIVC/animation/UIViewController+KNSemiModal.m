...
-(CAAnimationGroup*)animationGroupForward:(BOOL)_forward {
    ...
    t1 = CATransform3DScale(t1, 0.95, 0.95, 1);//等比缩小
    ...
    CGFloat height = [self parentTarget].frame.size.height;
    //这一步非常关键，这让上面只露出20像素，即statusbar
    CGFloat offset = (20 - height * 0.2/2); //-(20/0.95+height*pri)*0.8;
    t2 = CATransform3DTranslate(t2, 0, offset, 0);
    
    //    t2 = CATransform3DTranslate(t2, 0, [self parentTarget].frame.size.height*-0.08, 0);
    t2 = CATransform3DScale(t2, 0.80, 0.80, 1);//等比缩小
...
-(void)dismissSemiModalView {
    ...
    if (overlay.subviews.count) {
        ...
        [overlay.layer addAnimation:[self animationGroupForward:NO] forKey:@"bringForwardAnimation"];
        [UIView animateWithDuration:kSemiModalAnimationDuration animations:^{
            overlay.alpha = 1;
        }];
    }
